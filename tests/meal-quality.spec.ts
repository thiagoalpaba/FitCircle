...
function extractOptionTitles(section: string) {
  ...
  return section
    ...
    .filter((line) => {
      ...
      // Ignora valores dos cards de macros: 6G, 64G, 3G
      if (/^\d+G$/i.test(upper)) return false;

      // Ignora chips antigos: P 32G, C 48G, G 12G
      if (/^[PCG]\s*\d+G$/i.test(upper)) return false;

      // Ignora caso os 3 chips antigos venham na mesma linha
