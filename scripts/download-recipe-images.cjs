...
const images = [
  ...
  ['bolo-fit-banana-aveia.jpg', 'https://images.unsplash.com/photo-1565958011703-44f9829ba187?auto=format&fit=crop&w=900&q=80'],
  ...
  // Essa imagem antiga do cuscuz estava dando 404.
  // Por enquanto usamos uma imagem neutra de banana/café da manhã para não quebrar o script.
  ['cuscuz-banana-chia.jpg', 'https://images.unsplash.com/photo-1528825871115-3581a5387919?auto=format&fit=crop&w=900&q=80'],
...
function download(url, filename) {
  ...
  return new Promise((resolve, reject) => {
    if (fs.existsSync(filePath)) {
      ...
      console.log(`Substituindo imagem antiga: ${filename}`);
    }
...
async function main() {
  console.log('\nBaixando e substituindo imagens em public/recipes...\n');
  ...
  for (const [filename, url] of images) {
    try {
      ...
      console.log(`Falhou, mas vou continuar: ${filename}`);
  ...
  if (failed.length > 0) {
    ...
    failed.forEach(filename => console.log(`- ${filename}`));
  } else {
